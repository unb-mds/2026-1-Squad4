...
| RF05 | Evolução do posicionamento no tempo | 🟠 Média | Visualizar linha do tempo com mudanças de posição detectadas, destacando viradas significativas. |
| RF06 | Filtros e busca avançada | 🟠 Média | Filtrar por parlamentar, partido, tema/etiqueta, período, UF e tipo de votação. |
| RF07 | Comparação entre parlamentares | 🟠 Média | Permitir comparação lado a lado de scores de coerência e evolução entre dois ou mais parlamentares. |
| RF08 | Painel de alertas de divergência | 🟠 Média | Destacar automaticamente casos de alta divergência entre discurso e voto para análise prioritária. |
| RF09 | Exportação de relatórios | 🟢 Baixa | Gerar relatórios em CSV/PDF por parlamentar, período ou tema, com scores e trechos justificando a análise. |
| RF10 | Histórico de legislaturas | 🟢 Baixa | Suporte a múltiplas legislaturas para permitir análise longitudinal de mandatos anteriores. |
